...
function Homepage() {
  ...
  const searchCountry = (input) => {
    ...
    const filtered = countriesData.filter(country => {
      ...
      if (countryName.toLowerCase().includes(searchTerm)) return true;
      if (country.capital && country.capital[0].toLowerCase().includes(searchTerm)) return true;
      if (country.region.toLowerCase().includes(searchTerm)) return true;
      if (country.population.toString().includes(searchTerm)) return true;
      
      if (country.currencies) {
        const currencies = Object.values(country.currencies);
        if (currencies.some(curr => curr.name.toLowerCase().includes(searchTerm))) return true;
      }
      
      if (country.languages) {
        const languages = Object.values(country.languages);
        if (languages.some(lang => lang.toLowerCase().includes(searchTerm))) return true;
      }
      
      const additionalInfo = additionalCountryInfo[countryName];
      if (additionalInfo) {
        if (additionalInfo.funFact.toLowerCase().includes(searchTerm) ||
            additionalInfo.historicalFact.toLowerCase().includes(searchTerm)) {
          return true;
        }
      }
      ...
      if (cuisineInfo && cuisineInfo.localCusine) {
        if (cuisineInfo.localCusine.some(cuisine => cuisine.toLowerCase().includes(searchTerm))) {
          return true;
        }
      }
  ...
  const downloadFlag = (url, countryName) => {
    ...
    <div className="container-fluid">
      <div className="row">
        <div className="col-12 text-center my-3">
          <h1 className="animate__animated animate__fadeInDown">{t('World Countries Information')}</h1>
          <input
            type="text"
            id="searchInput"
          ...
          <div className="navbar">
            <div className="filter-sort">
              ...
              >
                <option value="">{t('Filter by Continent')}</option>
          ...
          <div className="text-center my-3">
            <Link to="/quiz" className="btn btn-primary">{t('Country Quiz')}</Link>
            <Link to="/visualization" className="btn btn-primary">{t('View Data Visualization')}</Link>
          </div>
        </div>
      </div>
      <div className="row" id="countryList">
        {filteredCountries.map(country => {
          ...
          const localCuisine = additionalInfo2[countryName] ? additionalInfo2[countryName].localCusine.join(', ') : 'No local cuisine available.';

          return (
            <div key={countryName} className="col-md-3 country-card animate__animated animate__fadeInUp">
              <h5>{countryName}</h5>
              <img src={country.flags.png} alt={`Flag of ${countryName}`} className="img-fluid" />
              ...
              <button className="btn btn-primary" onClick={() => downloadFlag(country.flags.png, countryName)}>Download Flag</button>
            </div>
          );
        })}
      </div>
